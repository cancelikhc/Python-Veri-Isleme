...
def read_documents(file_path):
    ...
    return documents
# Ana klasör yolu
base_path = r"C:\Users\LENOVO\Desktop\MAK-ÖĞR\Fold"
...
num_positive_test = len(test_positive_documents) # Fix here
num_negative_test = len(test_negative_documents) # Fix here
...
tfidf_test_with_classes = np.column_stack((tfidf_matrix_test, np.array(test_classes, dtype=int))) # Fix here

# CSV dosyasına yazma
column_names = list(df_values.keys())
column_names.append('class_label') #class_label olarak clasın adını değiştir

# Eğitim verisi
df = pd.DataFrame(tfidf_with_classes, columns=column_names)
df['class_label'] = df['class_label'].astype(int)  # Sınıf etiketlerini integer'a dönüştür
df.to_csv(r"C:\Users\LENOVO\Desktop\MAK-ÖĞR\Fold\5\training.csv", index=False)

# Test verisi
df_test = pd.DataFrame(tfidf_test_with_classes, columns=column_names)
df_test['class_label'] = df_test['class_label'].astype(int)  # Sınıf etiketlerini integer'a dönüştür
df_test.to_csv(r"C:\Users\LENOVO\Desktop\MAK-ÖĞR\Fold\5\test.csv", index=False)

print("Number of training documents:", len(all_training_documents))
print("Number of test documents:", len(all_test_documents))
